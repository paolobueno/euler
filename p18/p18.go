...
func main() {
	...
	for i := range longestLenghts {
		longestLenghts[i] = make([]int, len(problem[i]))
	}


	longestLenghts[0][0] = problem[0][0]
	for depth := 1; depth < len(problem); depth++ {
		arr := problem[depth]
	...
	result := helpers.MaxInSlice(longestLenghts[len(longestLenghts)-1])
	fmt.Println(result)
}